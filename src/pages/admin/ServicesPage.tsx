...
export function ServicesPage() {
  ...
  async function handleSavePricing() {
    ...
    try {
      ...
      const desc = customDescription.trim() || null;
      await setCustomPricing(selectedService.service, rate, margin, isEnabled, desc);
  ...
  async function handleEnableService(s: ServiceActivity) {
    try {
      await setCustomPricing(s.service_id, s.custom_rate, s.profit_margin, true, s.custom_description);
      ...
      {selectedService && (
        <div className="modal-overlay" onClick={() => setSelectedService(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              <label className="form-label">Service Description / Instructions</label>
              <textarea
                className="form-input"
                style={{ minHeight: '80px', resize: 'vertical', fontFamily: 'inherit', padding: '8px 12px' }}
                placeholder="Enter detailed description/instructions for the user..."
